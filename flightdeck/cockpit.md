...
**Last updated**: 2026-06-23 by 月离 (v1.9.0 已发布 push+tag：图片类目配置表驱动重构——single/grid/external 三 kind 由 [[image.entry]] 驱动，arg 显式表达添加规则，external 支持 pick 显式下标[N]/[*]/[random]+base 补全相对路径，语录/表情抽独立插件，help 由配置生成；契约见 docs/image-categories-config-table.md)
**Active focus**: v1.9.0 已发布（push + tag）：图片类目配置表驱动重构；连同 v1.7.0/v1.8.0 共四项待重新部署后手验（见 Pending Review / 下一步）；无进行中开发任务
...
4. **图片类目配置表重构（v1.9.0）** — 默认表不配则行为不变；验证：龙图/福瑞等随机一张、吃啥/喝啥等 4合1 网格（添加吃喝玩乐须带名字 `添加吃的 麻辣烫`、4合1 显示名字、同名不覆盖）、随机猫猫外链、添加语录/表情/各类图、`帮助 龙图`/`帮助 添加语录` 能查到；新配的 `随机插画`（external pick=`data[*].url` + base 取 pln 画站）能发图。契约见 `docs/image-categories-config-table.md`。
