...
**Last updated**: 2026-06-18 by 月离 (v1.7.0 已发布 push+tag：可配置 AI 频率限制 [ai.ratelimit] 每人/每群双窗口默认 5/15 + 加群审核命令改覆盖语义)
**Active focus**: v1.7.0 已发布（push + tag）：可配置 AI 调用频率限制（每人+每群双窗口）+ 加群审核命令改覆盖语义；两项均待重新部署后手验（见 Pending Review / 下一步）；无进行中开发任务
