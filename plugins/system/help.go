...
var pluginRegistry = []pluginEntry{
	...
	{3, "加群审核", "群管",
		"后台支持全局默认 / 单群覆盖 / 留人工；拒绝优先，未命中留人工",
		"  加群审核              查看本群模式与生效黑白名单\n" +
			"  加群白名单 词1,词2     切换本群独立配置并覆盖通过词（逗号多词；留空清空；* 匹配非空理由）\n" +
			"  加群黑名单 词1,词2     切换本群独立配置并覆盖拒绝词（留空清空）",
	...
	{41, "游戏折扣", "游戏",
		"查询 Epic 本周限免，以及 Steam 国区当前价和参考史低",
	...
	{43, "月灵自制表情包", "娱乐",
		...
		"  单身 + 图片 / @某人          保留上半部分，替换下半部分图片\n" +
			"  嘲笑 + 图片 / @某人          替换九宫格中心图片\n" +
			"  老人微笑 上句|下句           给上下照片添加底部字幕\n" +
			"  蜘蛛人戴眼镜 上句|下句       文字居中放入右侧两个区域\n" +
			"  游戏王 标题|说明 + 图片/@某人  生成游戏王卡牌\n" +
			"  握手洗手 第一段|第二段|第三段  生成握手洗手三联图\n\n" +
			"  图片优先级：附图 > 引用消息图 > @用户头像 > 发送者头像\n" +
			"  需要图片但未提供时，默认使用发送者头像",
		[]string{"单身", "嘲笑", "老人微笑", "蜘蛛人戴眼镜", "游戏王", "握手洗手"}},
	...
	{38, "订阅中心", "工具",
		...
		"  订阅添加 <URL> [名称]    添加订阅并从当前最新内容开始（群管理员）\n" +
			...
			"  订阅小红书 <用户ID/主页> [名称]  订阅用户笔记（群管理员）\n" +
			"  订阅GitHub <owner/repo> [名称]   订阅版本发布（群管理员）\n" +
			"  订阅GitHubIssues <owner/repo> [名称] 订阅 Issues（群管理员）\n" +
			...
			"  默认每 5 分钟检查，后台可按群调整检查间隔。\n" +
			"  订阅检查                 立即检查；非静默时段同步推送（群管理员）\n" +
			"  订阅删除 <ID>            删除订阅（群管理员）\n" +
			"  每群最多 10 个，每 10 分钟自动检查；失败自动退避并持久重试",
		[]string{"订阅添加", "订阅B站视频", "订阅B站动态", "订阅B站直播", "订阅X", "订阅小红书", "订阅GitHub", "订阅GitHubIssues", "订阅列表", "订阅状态", "订阅暂停", "订阅恢复", "订阅静默", "订阅检查", "订阅删除"}},
	...
	{40, "词云与聊天统计", "娱乐",
		...
		"  词云 / 今日词云       今日群聊词云\n" +
			...
			"  我的词云              自己今日的词云\n" +
			"  今日废话榜 / 本周龙王  群友消息数排行\n" +
			"  口头禅 [@群友/QQ号]   查自己或其他群友最近七天常说原句与高频词\n" +
